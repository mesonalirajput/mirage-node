...
| 2026-04-16 | Post Details Iteration 2: new `inboxHighlightRail`/`inboxHighlightBg` token pair (retired raw `#FACC15`). Comments header row with count + sort dropdown. Chevron collapse affordance. Compact 28px action pills with `feedCtrlText` rest color. Composer padding tightened. StateBlock pattern for loading/error/empty states. LoggedOutPromptCard for guest users. Block/success/confirm messages routed through R2 tokens. |
